...
## Terrain colours, pulled out of terrain.json into flat arrays the mesh builder can index without
## a dictionary lookup per vertex.
##
## The one colour that is not per-terrain is the cliff. Escarpment walls get their own colour so a
...
static func from_config(cfg: Config) -> Palette:
	...
	p.ground.resize(n)
	p.wall.resize(n)
	# Colours are authored as sRGB hex in terrain.json because that is what a human can read and
	# pick. The renderer works in linear, and vertex colours handed to a shader are taken as linear
	# as they stand — so the conversion has to happen here, once, rather than being approximated in
	# the shader on every fragment.
	for k: int in n:
		var c: Color = cfg.terrain_colours[k].srgb_to_linear()
		p.ground[k] = c
	...
	p.cliff = cfg.cliff_colour.srgb_to_linear()
	var water_type: int = cfg.type_by_name("water")
	if water_type >= 0:
		p.water = cfg.terrain_colours[water_type].srgb_to_linear()
	return p


## Slight per-tile tonal variation so a large field of one terrain type does not read as a flat
## sheet of colour. Deterministic from the tile index — no RNG, and identical every time the mesh
## is rebuilt.
